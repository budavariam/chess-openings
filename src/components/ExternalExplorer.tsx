...
const ResponsiveButton: React.FC<ResponsiveButtonProps> = ({
  ...
  <button
    onClick={onClick}
    className={`px-3 sm:py-1.5 py-2 text-sm rounded-md font-medium transition-colors min-w-0 overflow-hidden text-ellipsis whitespace-nowrap ${className}`}
    title={title}
  >
    {children}
  </button>
...
const ResponsiveButtonGroup: React.FC<ResponsiveButtonGroupProps> = ({
  ...
  <div className="space-y-2">
    ...
    <div
      className={`sm:hidden min-w-0 ${mobileLayout === "grid" ? "grid grid-cols-2 gap-2" : "flex flex-col gap-2"}`}
    >
...
export const ExternalExplorer: React.FC<ExternalExplorerProps> = ({
  ...
  const openInExplorer = useCallback(
    (site: "lichess" | "chess.com" | "365chess") => {
      if (!matchedOpening) {
        toast.error("No opening selected");
        return;
      }

      const moves = matchedOpening.moves.slice(
        0,
      ...
      switch (site) {
        ...
        case "365chess": {
          ...
          navigator.clipboard
            ?.writeText(moveText365)
            .then(() => {
              toast.info(
                `365chess - moves copied to clipboard: ${moveText365}`,
              );
            })
            .catch(() => {
